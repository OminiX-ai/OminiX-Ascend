...
- [ ] **Q1.10 Mixed-quant repack extension** — amended 2026-04-23 per Q2.1 fallback audit (`docs/qie_q21_fallback_audit.md`, commit `d523910b`). The GGUF filename `Qwen-Image-Edit-2509-Q4_0.gguf` is misleading: it's actually a **mixed-quant schedule** typical of `llama-quantize` — sensitive first/last layers (0, 59) use Q5_K for all 14 matmuls; mid-layers (1-58) use Q4_1 for FFN-down (`img_mlp.net.2`, `txt_mlp.net.2`); 6 globals in BF16; remainder Q4_0. Q2.1 smoke RED'd at 17.74 GiB because 150 tensors (Q4_1 + Q5_K + BF16) fell through the Q4_0-only repack path into F16 dequant (9.5 GiB). **Extension scope**: (a) generalize `repack_q4_0_to_wqbmmv3` → `repack_q4x_to_wqbmmv3` handling both Q4_0 and Q4_1 (Q4_1 = Q4_0 + one F16 min per block-of-32; ~30 LoC + one broadcast-add at matmul). Eliminates 116/150 fallbacks, saves 5.6 GiB. (b) Q5_K native repack is more complex kernel — defer as F16 fallback for now (1.27 GiB carry). (c) BF16 globals (75 MiB) stay F16, not worth code. **Gate re-scope: peak HBM ≤ 13 GiB** (was 9 GiB based on false uniform-Q4_0 assumption). Fits 32 GiB 910B4 with 19 GiB margin for activations + CFG duplication. Option-b re-export rejected (exporter flagged sensitive layers for quality); option-c accept-9.5-GiB rejected (leaves 6.4 GiB on table permanently).

**Gate**: end-to-end native-engine generates valid output image on canonical task. steps/s within ~20% of ggml-cann baseline (not a lift yet — just parity proven on native path). RoPE pre-compute verified at session init (not re-computed per step). **Q4-resident + Q4_1 native repack strategy verified to fit HBM at ≤ 13 GiB peak (target gate, amended 2026-04-23). Q5_K F16-fallback acceptable at 1.3 GiB carry.**

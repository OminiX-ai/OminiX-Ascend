#!/usr/bin/env bash
# Q2 Phase 4.2 60-block DiT smoke — build + run on ac03.
...
# Same build recipe as the Phase 3 block probe. Engine links against ggml
# static libs from the repo's existing build dir (for gguf parsing — not
# used on this path since init_for_smoke skips GGUF, but the engine TU
# still references ggml.h/gguf.h and needs to resolve their symbols).
GGML_BUILD=${GGML_BUILD:-$REPO/build-85}
if [ ! -d "$GGML_BUILD" ]; then
  echo "[smoke42] missing ggml build at $GGML_BUILD — set GGML_BUILD to a repo build dir"
  exit 1
fi
...
g++ -std=c++17 -O2 -g \
    ...
    -o test_qie_q42_60block_smoke \
    test_qie_q42_60block_smoke.cpp \
    "$ENGINE_SRC" \
    "$SYMS_SRC" \
...
if [ -e "$LOCK" ]; then
  echo "[smoke42] HBM lock present at $LOCK — proceeding (script re-touches)"
fi
touch "$LOCK"
trap "rm -f $LOCK" EXIT

./test_qie_q42_60block_smoke
